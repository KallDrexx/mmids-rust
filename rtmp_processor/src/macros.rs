macro_rules! assert_match(
    ($e:expr, $p:pat if $cond:expr => $e2:expr) => (
        match $e {
            $p if $cond => $e2,
            _ => panic!(concat!("Match failed: ", stringify!($e), " = ", stringify!($p))) 
        }
    );

    ($e:expr, $p:pat if $cond:expr) => (
        assert_matches!($e, $p, $cond, ());
    );

    ($e:expr, $p:pat => $e2:expr) => (
        assert_matches!($e, $p if true => $e2);
    );

    ($e:expr, $p:pat) => (
        assert_matches!($e, $p if true => ());
    )
);
